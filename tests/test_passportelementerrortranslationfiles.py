...
class TestPassportElementErrorTranslationFiles:
    ...
    def test_extra_slots(self, passport_element_error_translation_files):
        members = inspect.getmembers(
            passport_element_error_translation_files.__class__,
            predicate=lambda b: not inspect.isroutine(b) and (inspect.ismemberdescriptor(b)),
        )
        for member in members:
            val = getattr(passport_element_error_translation_files, member[0], 'err')
