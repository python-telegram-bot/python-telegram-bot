...
class TestPTBRequestBase:
    def test_replaced_unprintable_char(self):
        """
        ...
        assert PtbRequestBase._parse(server_response) == 'KUKU'

    def test_parse_illegal_json(self):
        """
        Clients can send arbitrary bytes in callback data.
        Make sure the correct error is raised in this case.
        """
        ...
        with pytest.raises(TelegramError, match='Invalid server response'):
            PtbRequestBase._parse(server_response)
