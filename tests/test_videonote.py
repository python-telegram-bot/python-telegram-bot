...
class TestVideoNote:
    ...
    def test_get_file_instance_method(self, monkeypatch, video_note):
        ...
        assert check_shortcut_call(video_note.get_file, video_note._bot, 'get_file')
        assert check_defaults_handling(video_note.get_file, video_note._bot)

        monkeypatch.setattr(video_note._bot, 'get_file', make_assertion)
