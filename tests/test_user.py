...
class TestUser:
    ...
    def test_instance_method_get_profile_photos(self, monkeypatch, user):
        ...
        assert check_shortcut_call(user.get_profile_photos, user._bot, 'get_user_profile_photos')
        assert check_defaults_handling(user.get_profile_photos, user._bot)

        monkeypatch.setattr(user._bot, 'get_user_profile_photos', make_assertion)
    ...
    def test_instance_method_pin_message(self, monkeypatch, user):
        ...
        assert check_shortcut_call(user.pin_message, user._bot, 'pin_chat_message')
        assert check_defaults_handling(user.pin_message, user._bot)

        monkeypatch.setattr(user._bot, 'pin_chat_message', make_assertion)
    ...
    def test_instance_method_unpin_message(self, monkeypatch, user):
        ...
        assert check_shortcut_call(user.unpin_message, user._bot, 'unpin_chat_message')
        assert check_defaults_handling(user.unpin_message, user._bot)

        monkeypatch.setattr(user._bot, 'unpin_chat_message', make_assertion)
    ...
    def test_instance_method_unpin_all_messages(self, monkeypatch, user):
        ...
        assert check_shortcut_call(user.unpin_all_messages, user._bot, 'unpin_all_chat_messages')
        assert check_defaults_handling(user.unpin_all_messages, user._bot)

        monkeypatch.setattr(user._bot, 'unpin_all_chat_messages', make_assertion)
    ...
    def test_instance_method_send_message(self, monkeypatch, user):
        ...
        assert check_shortcut_call(user.send_message, user._bot, 'send_message')
        assert check_defaults_handling(user.send_message, user._bot)

        monkeypatch.setattr(user._bot, 'send_message', make_assertion)
    ...
    def test_instance_method_send_photo(self, monkeypatch, user):
        ...
        assert check_shortcut_call(user.send_photo, user._bot, 'send_photo')
        assert check_defaults_handling(user.send_photo, user._bot)

        monkeypatch.setattr(user._bot, 'send_photo', make_assertion)
    ...
    def test_instance_method_send_media_group(self, monkeypatch, user):
        ...
        assert check_shortcut_call(user.send_media_group, user._bot, 'send_media_group')
        assert check_defaults_handling(user.send_media_group, user._bot)

        monkeypatch.setattr(user._bot, 'send_media_group', make_assertion)
    ...
    def test_instance_method_send_audio(self, monkeypatch, user):
        ...
        assert check_shortcut_call(user.send_audio, user._bot, 'send_audio')
        assert check_defaults_handling(user.send_audio, user._bot)

        monkeypatch.setattr(user._bot, 'send_audio', make_assertion)
    ...
    def test_instance_method_send_chat_action(self, monkeypatch, user):
        ...
        assert check_shortcut_call(user.send_chat_action, user._bot, 'send_chat_action')
        assert check_defaults_handling(user.send_chat_action, user._bot)

        monkeypatch.setattr(user._bot, 'send_chat_action', make_assertion)
    ...
    def test_instance_method_send_contact(self, monkeypatch, user):
        ...
        assert check_shortcut_call(user.send_contact, user._bot, 'send_contact')
        assert check_defaults_handling(user.send_contact, user._bot)

        monkeypatch.setattr(user._bot, 'send_contact', make_assertion)
    ...
    def test_instance_method_send_dice(self, monkeypatch, user):
        ...
        assert check_shortcut_call(user.send_dice, user._bot, 'send_dice')
        assert check_defaults_handling(user.send_dice, user._bot)

        monkeypatch.setattr(user._bot, 'send_dice', make_assertion)
    ...
    def test_instance_method_send_document(self, monkeypatch, user):
        ...
        assert check_shortcut_call(user.send_document, user._bot, 'send_document')
        assert check_defaults_handling(user.send_document, user._bot)

        monkeypatch.setattr(user._bot, 'send_document', make_assertion)
    ...
    def test_instance_method_send_game(self, monkeypatch, user):
        ...
        assert check_shortcut_call(user.send_game, user._bot, 'send_game')
        assert check_defaults_handling(user.send_game, user._bot)

        monkeypatch.setattr(user._bot, 'send_game', make_assertion)
    ...
    def test_instance_method_send_invoice(self, monkeypatch, user):
        ...
        assert check_shortcut_call(user.send_invoice, user._bot, 'send_invoice')
        assert check_defaults_handling(user.send_invoice, user._bot)

        monkeypatch.setattr(user._bot, 'send_invoice', make_assertion)
    ...
    def test_instance_method_send_location(self, monkeypatch, user):
        ...
        assert check_shortcut_call(user.send_location, user._bot, 'send_location')
        assert check_defaults_handling(user.send_location, user._bot)

        monkeypatch.setattr(user._bot, 'send_location', make_assertion)
    ...
    def test_instance_method_send_sticker(self, monkeypatch, user):
        ...
        assert check_shortcut_call(user.send_sticker, user._bot, 'send_sticker')
        assert check_defaults_handling(user.send_sticker, user._bot)

        monkeypatch.setattr(user._bot, 'send_sticker', make_assertion)
    ...
    def test_instance_method_send_video(self, monkeypatch, user):
        ...
        assert check_shortcut_call(user.send_video, user._bot, 'send_video')
        assert check_defaults_handling(user.send_video, user._bot)

        monkeypatch.setattr(user._bot, 'send_video', make_assertion)
    ...
    def test_instance_method_send_venue(self, monkeypatch, user):
        ...
        assert check_shortcut_call(user.send_venue, user._bot, 'send_venue')
        assert check_defaults_handling(user.send_venue, user._bot)

        monkeypatch.setattr(user._bot, 'send_venue', make_assertion)
    ...
    def test_instance_method_send_video_note(self, monkeypatch, user):
        ...
        assert check_shortcut_call(user.send_video_note, user._bot, 'send_video_note')
        assert check_defaults_handling(user.send_video_note, user._bot)

        monkeypatch.setattr(user._bot, 'send_video_note', make_assertion)
    ...
    def test_instance_method_send_voice(self, monkeypatch, user):
        ...
        assert check_shortcut_call(user.send_voice, user._bot, 'send_voice')
        assert check_defaults_handling(user.send_voice, user._bot)

        monkeypatch.setattr(user._bot, 'send_voice', make_assertion)
    ...
    def test_instance_method_send_animation(self, monkeypatch, user):
        ...
        assert check_shortcut_call(user.send_animation, user._bot, 'send_animation')
        assert check_defaults_handling(user.send_animation, user._bot)

        monkeypatch.setattr(user._bot, 'send_animation', make_assertion)
    ...
    def test_instance_method_send_poll(self, monkeypatch, user):
        ...
        assert check_shortcut_call(user.send_poll, user._bot, 'send_poll')
        assert check_defaults_handling(user.send_poll, user._bot)

        monkeypatch.setattr(user._bot, 'send_poll', make_assertion)
    ...
    def test_instance_method_send_copy(self, monkeypatch, user):
        ...
        assert check_shortcut_call(user.copy_message, user._bot, 'copy_message')
        assert check_defaults_handling(user.copy_message, user._bot)

        monkeypatch.setattr(user._bot, 'copy_message', make_assertion)
