...
class TestPassportElementErrorReverseSide:
    ...
    def test_extra_slots(self, passport_element_error_reverse_side):
        members = inspect.getmembers(
            passport_element_error_reverse_side.__class__,
            predicate=lambda b: not inspect.isroutine(b) and (inspect.ismemberdescriptor(b)),
        )
        for member in members:
            val = getattr(passport_element_error_reverse_side, member[0], 'err')
