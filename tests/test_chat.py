...
class TestChat:
    ...
    def test_send_action(self, monkeypatch, chat):
        ...
        assert check_shortcut_call(chat.send_action, chat._bot, 'send_chat_action')
        assert check_defaults_handling(chat.send_action, chat._bot)

        monkeypatch.setattr(chat._bot, 'send_chat_action', make_assertion)
    ...
    def test_leave(self, monkeypatch, chat):
        ...
        assert check_shortcut_call(chat.leave, chat._bot, 'leave_chat')
        assert check_defaults_handling(chat.leave, chat._bot)

        monkeypatch.setattr(chat._bot, 'leave_chat', make_assertion)
        assert chat.leave()
    ...
    def test_get_administrators(self, monkeypatch, chat):
        ...
        assert check_shortcut_call(chat.get_administrators, chat._bot, 'get_chat_administrators')
        assert check_defaults_handling(chat.get_administrators, chat._bot)

        monkeypatch.setattr(chat._bot, 'get_chat_administrators', make_assertion)
    ...
    def test_get_member_count(self, monkeypatch, chat):
        ...
        assert check_shortcut_call(chat.get_member_count, chat._bot, 'get_chat_member_count')
        assert check_defaults_handling(chat.get_member_count, chat._bot)

        monkeypatch.setattr(chat._bot, 'get_chat_member_count', make_assertion)
    ...
    def test_get_member(self, monkeypatch, chat):
        ...
        assert check_shortcut_call(chat.get_member, chat._bot, 'get_chat_member')
        assert check_defaults_handling(chat.get_member, chat._bot)

        monkeypatch.setattr(chat._bot, 'get_chat_member', make_assertion)
    ...
    def test_ban_member(self, monkeypatch, chat):
        ...
        assert check_shortcut_call(chat.ban_member, chat._bot, 'ban_chat_member')
        assert check_defaults_handling(chat.ban_member, chat._bot)

        monkeypatch.setattr(chat._bot, 'ban_chat_member', make_assertion)
    ...
    def test_unban_member(self, monkeypatch, chat, only_if_banned):
        ...
        assert check_shortcut_call(chat.unban_member, chat._bot, 'unban_chat_member')
        assert check_defaults_handling(chat.unban_member, chat._bot)

        monkeypatch.setattr(chat._bot, 'unban_chat_member', make_assertion)
    ...
    def test_promote_member(self, monkeypatch, chat, is_anonymous):
        ...
        assert check_shortcut_call(chat.promote_member, chat._bot, 'promote_chat_member')
        assert check_defaults_handling(chat.promote_member, chat._bot)

        monkeypatch.setattr(chat._bot, 'promote_chat_member', make_assertion)
    ...
    def test_restrict_member(self, monkeypatch, chat):
        ...
        assert check_shortcut_call(chat.restrict_member, chat._bot, 'restrict_chat_member')
        assert check_defaults_handling(chat.restrict_member, chat._bot)

        monkeypatch.setattr(chat._bot, 'restrict_chat_member', make_assertion)
    ...
    def test_set_permissions(self, monkeypatch, chat):
        ...
        assert check_shortcut_call(chat.set_permissions, chat._bot, 'set_chat_permissions')
        assert check_defaults_handling(chat.set_permissions, chat._bot)

        monkeypatch.setattr(chat._bot, 'set_chat_permissions', make_assertion)
    ...
    def test_pin_message(self, monkeypatch, chat):
        ...
        assert check_shortcut_call(chat.pin_message, chat._bot, 'pin_chat_message')
        assert check_defaults_handling(chat.pin_message, chat._bot)

        monkeypatch.setattr(chat._bot, 'pin_chat_message', make_assertion)
    ...
    def test_unpin_message(self, monkeypatch, chat):
        ...
        assert check_shortcut_call(chat.unpin_message, chat._bot, 'unpin_chat_message')
        assert check_defaults_handling(chat.unpin_message, chat._bot)

        monkeypatch.setattr(chat._bot, 'unpin_chat_message', make_assertion)
    ...
    def test_unpin_all_messages(self, monkeypatch, chat):
        ...
        assert check_shortcut_call(chat.unpin_all_messages, chat._bot, 'unpin_all_chat_messages')
        assert check_defaults_handling(chat.unpin_all_messages, chat._bot)

        monkeypatch.setattr(chat._bot, 'unpin_all_chat_messages', make_assertion)
    ...
    def test_instance_method_send_message(self, monkeypatch, chat):
        ...
        assert check_shortcut_call(chat.send_message, chat._bot, 'send_message')
        assert check_defaults_handling(chat.send_message, chat._bot)

        monkeypatch.setattr(chat._bot, 'send_message', make_assertion)
    ...
    def test_instance_method_send_media_group(self, monkeypatch, chat):
        ...
        assert check_shortcut_call(chat.send_media_group, chat._bot, 'send_media_group')
        assert check_defaults_handling(chat.send_media_group, chat._bot)

        monkeypatch.setattr(chat._bot, 'send_media_group', make_assertion)
    ...
    def test_instance_method_send_photo(self, monkeypatch, chat):
        ...
        assert check_shortcut_call(chat.send_photo, chat._bot, 'send_photo')
        assert check_defaults_handling(chat.send_photo, chat._bot)

        monkeypatch.setattr(chat._bot, 'send_photo', make_assertion)
    ...
    def test_instance_method_send_contact(self, monkeypatch, chat):
        ...
        assert check_shortcut_call(chat.send_contact, chat._bot, 'send_contact')
        assert check_defaults_handling(chat.send_contact, chat._bot)

        monkeypatch.setattr(chat._bot, 'send_contact', make_assertion)
    ...
    def test_instance_method_send_audio(self, monkeypatch, chat):
        ...
        assert check_shortcut_call(chat.send_audio, chat._bot, 'send_audio')
        assert check_defaults_handling(chat.send_audio, chat._bot)

        monkeypatch.setattr(chat._bot, 'send_audio', make_assertion)
    ...
    def test_instance_method_send_document(self, monkeypatch, chat):
        ...
        assert check_shortcut_call(chat.send_document, chat._bot, 'send_document')
        assert check_defaults_handling(chat.send_document, chat._bot)

        monkeypatch.setattr(chat._bot, 'send_document', make_assertion)
    ...
    def test_instance_method_send_dice(self, monkeypatch, chat):
        ...
        assert check_shortcut_call(chat.send_dice, chat._bot, 'send_dice')
        assert check_defaults_handling(chat.send_dice, chat._bot)

        monkeypatch.setattr(chat._bot, 'send_dice', make_assertion)
    ...
    def test_instance_method_send_game(self, monkeypatch, chat):
        ...
        assert check_shortcut_call(chat.send_game, chat._bot, 'send_game')
        assert check_defaults_handling(chat.send_game, chat._bot)

        monkeypatch.setattr(chat._bot, 'send_game', make_assertion)
    ...
    def test_instance_method_send_invoice(self, monkeypatch, chat):
        ...
        assert check_shortcut_call(chat.send_invoice, chat._bot, 'send_invoice')
        assert check_defaults_handling(chat.send_invoice, chat._bot)

        monkeypatch.setattr(chat._bot, 'send_invoice', make_assertion)
    ...
    def test_instance_method_send_location(self, monkeypatch, chat):
        ...
        assert check_shortcut_call(chat.send_location, chat._bot, 'send_location')
        assert check_defaults_handling(chat.send_location, chat._bot)

        monkeypatch.setattr(chat._bot, 'send_location', make_assertion)
    ...
    def test_instance_method_send_sticker(self, monkeypatch, chat):
        ...
        assert check_shortcut_call(chat.send_sticker, chat._bot, 'send_sticker')
        assert check_defaults_handling(chat.send_sticker, chat._bot)

        monkeypatch.setattr(chat._bot, 'send_sticker', make_assertion)
    ...
    def test_instance_method_send_venue(self, monkeypatch, chat):
        ...
        assert check_shortcut_call(chat.send_venue, chat._bot, 'send_venue')
        assert check_defaults_handling(chat.send_venue, chat._bot)

        monkeypatch.setattr(chat._bot, 'send_venue', make_assertion)
    ...
    def test_instance_method_send_video(self, monkeypatch, chat):
        ...
        assert check_shortcut_call(chat.send_video, chat._bot, 'send_video')
        assert check_defaults_handling(chat.send_video, chat._bot)

        monkeypatch.setattr(chat._bot, 'send_video', make_assertion)
    ...
    def test_instance_method_send_video_note(self, monkeypatch, chat):
        ...
        assert check_shortcut_call(chat.send_video_note, chat._bot, 'send_video_note')
        assert check_defaults_handling(chat.send_video_note, chat._bot)

        monkeypatch.setattr(chat._bot, 'send_video_note', make_assertion)
    ...
    def test_instance_method_send_voice(self, monkeypatch, chat):
        ...
        assert check_shortcut_call(chat.send_voice, chat._bot, 'send_voice')
        assert check_defaults_handling(chat.send_voice, chat._bot)

        monkeypatch.setattr(chat._bot, 'send_voice', make_assertion)
    ...
    def test_instance_method_send_animation(self, monkeypatch, chat):
        ...
        assert check_shortcut_call(chat.send_animation, chat._bot, 'send_animation')
        assert check_defaults_handling(chat.send_animation, chat._bot)

        monkeypatch.setattr(chat._bot, 'send_animation', make_assertion)
    ...
    def test_instance_method_send_poll(self, monkeypatch, chat):
        ...
        assert check_shortcut_call(chat.send_poll, chat._bot, 'send_poll')
        assert check_defaults_handling(chat.send_poll, chat._bot)

        monkeypatch.setattr(chat._bot, 'send_poll', make_assertion)
    ...
    def test_instance_method_send_copy(self, monkeypatch, chat):
        ...
        assert check_shortcut_call(chat.copy_message, chat._bot, 'copy_message')
        assert check_defaults_handling(chat.copy_message, chat._bot)

        monkeypatch.setattr(chat._bot, 'copy_message', make_assertion)
    ...
    def test_export_invite_link(self, monkeypatch, chat):
        ...
        assert check_shortcut_call(chat.export_invite_link, chat._bot, 'export_chat_invite_link')
        assert check_defaults_handling(chat.export_invite_link, chat._bot)

        monkeypatch.setattr(chat._bot, 'export_chat_invite_link', make_assertion)
    ...
    def test_create_invite_link(self, monkeypatch, chat):
        ...
        assert check_shortcut_call(chat.create_invite_link, chat._bot, 'create_chat_invite_link')
        assert check_defaults_handling(chat.create_invite_link, chat._bot)

        monkeypatch.setattr(chat._bot, 'create_chat_invite_link', make_assertion)
    ...
    def test_edit_invite_link(self, monkeypatch, chat):
        ...
        assert check_shortcut_call(chat.edit_invite_link, chat._bot, 'edit_chat_invite_link')
        assert check_defaults_handling(chat.edit_invite_link, chat._bot)

        monkeypatch.setattr(chat._bot, 'edit_chat_invite_link', make_assertion)
    ...
    def test_revoke_invite_link(self, monkeypatch, chat):
        ...
        assert check_shortcut_call(chat.revoke_invite_link, chat._bot, 'revoke_chat_invite_link')
        assert check_defaults_handling(chat.revoke_invite_link, chat._bot)

        monkeypatch.setattr(chat._bot, 'revoke_chat_invite_link', make_assertion)
