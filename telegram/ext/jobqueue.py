...
class JobQueue(object):
    ...
    Attributes:
        queue (PriorityQueue): The queue that holds the Jobs.
    ...
    def put(self, job, next_t=None):
        ...
        Args:
            ...
            next_t (Optional[int|float|datetime.timedelta|datetime.datetime|datetime.time]):
                    ...
                    * ``int`` or ``float`` will be interpreted as "seconds from now" in which the
                    job should run.

                    * ``datetime.timedelta`` will be interpreted as "time from now" in which the
                    job should run.

                    * ``datetime.datetime`` will be interpreted as a specific date and time at
                    which the job should run.

                    * ``datetime.time`` will be interpreted as a specific time at which the job
    ...
    def run_once(self, callback, when, context=None, name=None):
        ...
        Args:
            callback (function): The callback function that should be executed by the new job. It
                    should take ``bot, job`` as parameters, where ``job`` is the
            ...
            when (int|float|datetime.timedelta|datetime.datetime|datetime.time):
                    ...
                    * ``datetime.time`` will be interpreted as a specific time of day at which the
...
class Job(object):
    ...
    Attributes:
        callback (function): The callback function that should be executed by the new job.
        interval (int|float|datetime.timedelta): Optional. The interval in which the job will run.
        repeat (bool): Optional. If this job should be periodically execute its callback function.
    ...
    Args:
        callback (function): The callback function that should be executed by the new job. It
                    should take ``bot, job`` as parameters, where ``job`` is the
                    :class:`telegram.ext.Job` instance. It can be used to access it's
                    :attr:`context` or change it to a repeating job.
        interval (Optional[int|float|datetime.timedelta]): The interval in which the job will run.
                    If it is an ``int`` or a ``float``, it will be interpreted as seconds. If you
                    don't set this value, you must set ``repeat=False`` and specify ``next_t`` when
