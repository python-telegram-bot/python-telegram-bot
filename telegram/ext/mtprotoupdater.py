...
from telegram.mtprotobot import MTProtoBot
...
class MTProtoUpdater(object):
    ...
    def __init__(self,
        ...
        if bot is not None:
            if not isinstance(bot, MTProtoBot):
                raise ValueError(f"{self.__class__.__name__} can only work "
                                 f"with {MTProtoBot.__name__}s.")
            self.bot = bot
            ...
            self.bot = MTProtoBot(token, base_url, request=self._request)
        ...
        self.peers_by_id = dict()
        self.peers_by_phone = dict()
        self.peers_by_username = dict()
    ...
    def load_session(self):
        try:
            with open(os.path.join(self.workdir, "{}.session".format(self.bot.token)),
                      encoding="utf-8") as f:
                s = json.load(f)
        except FileNotFoundError:
            self.dc_id = 1
            self.date = 0
            self.auth_key = Auth(self.dc_id, False, None).create()
        else:
            self.dc_id = s["dc_id"]
    ...
    def start_mtproto(self):
        with self.__lock:
            if not self.running:
                ...
                self._init_thread(self._start_mtproto, "mtproto-updater")
