...
* ``pip install python-telegram-bot[json]`` installs the `orjson <https://pypi.org/project/orjson/>`_ library. It will then be used for JSON de- & encoding, which can bring speed up compared to the standard `json <https://docs.python.org/3/library/json.html>`_ library.
